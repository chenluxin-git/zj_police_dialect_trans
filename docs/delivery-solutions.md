...
- **增量三件套（语音转译功能上线，2026-09-25 已部署）**，放 `dist-record/upload-20260925-212558/`：
  - `zjpdt-backend-20260925-212558.tar.gz` 235M（sha `8cf50301b7f32568`）
  - `record_web-20260925-212558.tar.gz` 505K（sha `232ee30316e18209`）
  - `deploy/record/update.sh`（与上版同 sha `aaf2e0bd2b842d27`，未改动）
  - 含 Git `7fb5f31..d8b0ef5` 共 8 提交：转译全功能（6081eed 泵竞态修复后）+
    播放重做 + 100MB 限额（前后端）+ moov-at-end mp4 转码修复 + 历史页/管理端层级筛选。
...
- **2026-09-25 增量上线（d8b0ef5）**：语音转译功能首次上生产（工作台/历史记录/管理端转译记录），
  含历史页与管理端层级查看（本级+下级、区域整域筛选）、视频上传上限 100MB（前后端预检 + nginx 200m 首装已有）、
  moov-at-end mp4/mov 转码修复、三视图播放重做。识别上游与录音质检共用 `.env` 的 `ASR_UPSTREAM_BASE`
  （隧道漂移时质检与转译一起换），部署后实测通过。
