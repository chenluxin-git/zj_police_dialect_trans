# tests/test_annotations.py —— T10 标注作业：3分钟锁/区域过滤/方言译文必填/一条音频一条标注/我的标注可改可删
...
def test_submit_ok_and_assignment_removed(client, db):
    ...
    r = client.post("/api/annotations", headers=auth(u),
                    json={"file_id": f.id, "is_dialect": True, "translation": "你好"})
    assert r.status_code == 200 and r.json()["code"] == 0
    ...
    r2 = client.post("/api/annotations", headers=auth(u),
                     json={"file_id": f.id, "is_dialect": False, "translation": ""})
    assert r2.status_code == 400


def test_submit_dialect_requires_translation(client, db):
    u = make_user(db)
    f = make_audio(db)
    take(client, u)
    r = client.post("/api/annotations", headers=auth(u),
                    json={"file_id": f.id, "is_dialect": True, "translation": ""})
...
def test_update_and_delete_own(client, db):
    ...
    client.post("/api/annotations", headers=auth(u),
                json={"file_id": f.id, "is_dialect": True, "translation": "旧译文"})
    ann = db.query(Annotation).filter_by(file_id=f.id).first()
    r = client.put(f"/api/annotations/{ann.id}", headers=auth(u),
                   json={"file_id": f.id, "is_dialect": False, "translation": ""})
    assert r.status_code == 200 and r.json()["code"] == 0
    assert ann.is_dialect is False and ann.translation == ""
...
def test_update_others_annotation_404(client, db):
    ...
    r = client.put(f"/api/annotations/{ann.id}", headers=auth(u2),
                   json={"file_id": f.id, "is_dialect": True, "translation": "x"})
    assert r.status_code == 404


def test_my_list_and_dialect_count(client, db):
    u = make_user(db)
    ...
    r1 = client.post("/api/annotations", headers=auth(u),
                     json={"file_id": d1["file_id"], "is_dialect": True, "translation": "你好"})
    assert r1.status_code == 200
    d2 = take(client, u)
    r2p = client.post("/api/annotations", headers=auth(u),
                      json={"file_id": d2["file_id"], "is_dialect": False, "translation": ""})
